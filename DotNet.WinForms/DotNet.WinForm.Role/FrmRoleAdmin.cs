...
using DotNet.Utilities.Information;
using DotNet.Utilities.ResourceManager;
using DotNet.WinForm.Common;
using DotNet.WinForm.Common.Interface;

namespace DotNet.WinForm.Role
{
    /// <summary>
    /// FrmRoleAdmin.cs
    /// 角色管理-角色管理窗体
    ...
    public partial class FrmRoleAdmin : BaseForm, IBaseForm
    {
        ...
        public int OverDelete() 
        {
            DataBase DB = new DataBase();

            string[] IDS = GetSelecteIds();
            ...
            System.Data.OleDb.OleDbParameter[] p = {
                               DB.MakeInParm("@DeletionStateCode",System.Data.OleDb.OleDbType.Integer,4,1 ),
                               DB.MakeInParm("@ModifiedUserId",System.Data.OleDb.OleDbType.VarChar,30,UserInfo.Id.ToString()),
                               DB.MakeInParm("@ModifiedOn",System.Data.OleDb.OleDbType.VarChar,50,DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"))
            };

            return DB.RunProc(" UPDATE BaseRole SET DeletionStateCode =  @DeletionStateCode , ModifiedUserId =  @ModifiedUserId , ModifiedOn = @ModifiedOn  WHERE Id in (" + ID + ")", p);
        }
