...
namespace DotNet.BaseManagers
{
   ...
   public partial class SQLBuilder
   {
      ...
      public void SetValue(string targetFiled, object targetValue, string targetFiledName = null)
      {
         ...
         switch (this.sqlOperation)
         {
            ...
            case DbOperation.Insert:
               ...
               else if (DbHelper.CurrentDbType == DbTypes.Access)
               {
                   if (this.Identity && targetFiled == this.PrimaryKey)
                   {
                       // 自增量，不需要赋值  
               ...
               {
                   if (this.Identity && targetFiled == this.PrimaryKey && DbHelper.CurrentDbType == DbTypes.SqlServer)
                  {
                     // 自增量，不需要赋值
                  }
                  else
      ...
      private int Execute()
      {
         ...
         if (this.Identity && this.sqlOperation == DbOperation.Insert && DbHelper.CurrentDbType == DbTypes.SqlServer)
         {
            // 读取返回值
            if (this.ReturnId)
            {
